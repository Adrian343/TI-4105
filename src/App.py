...
class App(tk.Frame):
    ...
    def __init__(self):
        ...
        self.spm = tk.Label(self.root, wraplength=self.ROOTWIDTH-25, font=("Times New Roman",20), justify="left")
        self.spm.pack(anchor="w")
        self.spm.bind("<Configure>", self.obj.adjustScrollregion)

        self.btnfrm = tk.Frame(self.root)
        self.btna = tk.Button(self.btnfrm, text="a", command=lambda: self.answer(self.btna), font=("Times New Roman",20), wraplength=self.ROOTWIDTH-40, justify="left")
        self.btnb = tk.Button(self.btnfrm, text="b", command=lambda: self.answer(self.btnb), font=("Times New Roman",20), wraplength=self.ROOTWIDTH-40, justify="left")
        self.btnc = tk.Button(self.btnfrm, text="c", command=lambda: self.answer(self.btnc), font=("Times New Roman",20), wraplength=self.ROOTWIDTH-40, justify="left")
        self.btnd = tk.Button(self.btnfrm, text="d", command=lambda: self.answer(self.btnd), font=("Times New Roman",20), wraplength=self.ROOTWIDTH-40, justify="left")
        ...
        self.LF = tk.Label(self.root, wraplength=self.ROOTWIDTH-25, justify="left")
    ...
    def configureWindow(self):
        ...
        self.window.geometry(f"{self.ROOTWIDTH}x{self.ROOTHEIGHT}+{self.ROOTX}+{self.ROOTY}")
        self.window.title("Løs TIØ4105 eksamen")
        self.window.maxsize(self.ROOTWIDTH, self.ROOTHEIGHT)
        self.obj = ScrollableFrame.ScrollableFrame(self.window,height=100, width=self.ROOTWIDTH) # Total required height of canvas,width=400 # Total width of master)
    ...
    def answer(self, button):
        if self.showingLF:
            ...
            if self.LFContent == "":
                self.LF["text"] = ""
                self.LFImage = tk.PhotoImage(file=path.join(path.dirname(__file__), f"resources/pictures/{self.currentQuestion}LF.png"))
                self.LF['image'] = self.LFImage
            else:
                self.LF["image"] = ""
            ...
            if button["text"] == self.correctAnswer:
                ...
                [v for v in self.btnfrm.children.values() if v["text"] == self.correctAnswer][0]["foreground"] = "GREEN"
                self.scoreChange(self.currentQuestion, -1)
    ...
    def getNextQuestion(self):
        ...
        if currQ[0] == "":
            self.spm["text"] = ""
            self.spmImage = tk.PhotoImage(file=path.join(path.dirname(__file__), f"resources/pictures/{self.currentQuestion}.png"))
    ...
    def fetchQuestions(self):
        ...
        with open(path.join(path.dirname(__file__), 'resources/Qs.txt'), "r") as file:
            ...
            for i in range(0, len(data), 6):
                self.questions.append([data[i], data[i+1], data[i+2], data[i+3], data[i+4], data[i+5]])
